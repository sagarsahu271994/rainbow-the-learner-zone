...
app/
  ...
  api/
    admissions/          Saves admission and private files
...
The SQL creates private `admission-documents` and `payment-screenshots` buckets. Public clients have no direct table access; all sensitive operations are performed by validated server routes.
...
- Submit a test admission, inspect private Storage, create a fee, and test lookup.
...
The included logo and classroom image were generated specifically for this project. They can be replaced without code changes by retaining the same filenames.
